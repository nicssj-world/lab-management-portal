...
const querySchema = z.object({
  q: z.string().trim().max(200).optional(), unit: z.string().trim().max(100).optional(),
...
export async function GET(request: NextRequest) {
  ...
  if (!parsed.success) return NextResponse.json({ error: 'Invalid filters' }, { status: 422, headers: { 'Cache-Control': 'no-store' } })
  const { page, pageSize, ...filters } = parsed.data
  const all = await searchPublicSds(filters)
  return NextResponse.json({ items: all.slice((page - 1) * pageSize, page * pageSize), count: all.length }, { headers: { 'Cache-Control': 'no-store' } })
}
