...
// POST — upload one or more attachment files
export async function POST(
  ...
  // Check document exists and status allows attachments
  const { data: doc } = await supabaseAdmin
    .from('documents').select('id, status').eq('id', id).single()
  if (!doc) return NextResponse.json({ error: 'ไม่พบเอกสาร' }, { status: 404 })
  if (!['Draft', 'Review', 'Approved', 'Published'].includes(doc.status)) {
    return NextResponse.json({ error: 'ไม่สามารถแนบไฟล์ในสถานะนี้ได้' }, { status: 422 })
  }
  ...
  try {
    ...
    for (const file of files) {
      ...
      const key = `documents/attachments/${id}/${Date.now()}-${safeName}`
