...
export const scheduleSchema = z.object({ id: z.string().optional().default(''), templateId: z.string().optional().default(''), intervalUnit: z.enum(['day','week','month','year']), intervalCount: z.number().int().positive(), recurrenceMode: z.enum(['fixed_calendar','rolling_completion']).default('fixed_calendar'), dueDayOfMonth: z.number().int().min(1).max(28).nullable().optional().default(null), startsOn: z.string().date(), endsOn: z.string().date().nullable(), active: z.boolean().default(true) })
