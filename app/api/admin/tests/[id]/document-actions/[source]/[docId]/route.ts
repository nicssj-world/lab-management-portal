...
export async function GET(req: NextRequest, { params }: Params) {
  ...
  if (!Number.isInteger(testId) || testId <= 0 || (action !== 'view' && action !== 'download')) {
    return NextResponse.json({ error: 'Invalid document action' }, { status: 422 })
  }
  ...
  if (source === 'library') {
    const { data: test } = await supabaseAdmin
      .from('tests')
      .select('related_doc_ids, related_doc_access')
      .eq('id', testId)
      .single()
    ...
    const { data: document } = await supabaseAdmin
      .from('documents')
      .select('file_url, file_name, visibility, document_code, title')
      .eq('id', docId)
      .is('deleted_at', null)
      .single()
    if (!document?.file_url) return NextResponse.json({ error: 'Not found' }, { status: 404 })

    const accessByDocument = (test?.related_doc_access ?? {}) as Record<string, string>
    const access = normalizeDocumentAccess(document.visibility, accessByDocument[docId])
    ...
    if (!Number.isInteger(documentId)) return NextResponse.json({ error: 'Invalid document id' }, { status: 422 })
    ...
    return NextResponse.json({ error: 'Invalid document source' }, { status: 422 })
  }
