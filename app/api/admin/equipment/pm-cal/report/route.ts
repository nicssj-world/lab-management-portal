...
import { buildPmCalReport, type PmCalReportEquipment, type PmCalReportPlan, type PmCalReportResult } from '@/lib/equipment/pm-cal-report'
...
export async function GET(req: NextRequest) {
  ...
  const [equipment, allPlans, allResults] = await Promise.all([
    ...
    fetchAllPages(async (from, to) => {
      const { data, error } = await supabaseAdmin.from('equipment_pm_cal_plans')
        .select('id, equipment_id, fiscal_year, calendar_month, cal_type, due_date, record_status, version, planned_cost')
  ...
  const report = buildPmCalReport({ equipment: equipment as unknown as PmCalReportEquipment[], plans, results })
