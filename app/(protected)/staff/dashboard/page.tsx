...
function actionDotColor(action: string | null): string {
  ...
  if (a.startsWith('risk.') || a.startsWith('rejection.') ||
      ...
      a.startsWith('smart_rm.'))                                  return '#DC2626' // red — ความเสี่ยง
  if (a.startsWith('lab_map.') || a.startsWith('chemical_safety.')) return '#059669' // emerald — ความปลอดภัย
...
export default async function StaffDashboardPage() {
  ...
  const [
    ...
    supabaseAdmin.from('profiles').select('*', { count: 'exact', head: true }).eq('status', 'active').is('deleted_at', null),
    supabaseAdmin.from('profiles').select('*', { count: 'exact', head: true }).eq('status', 'active').is('deleted_at', null).eq('role', 'Medical Technologist'),
    supabaseAdmin.from('profiles').select('mt_license_expiry').eq('status', 'active').is('deleted_at', null).eq('role', 'Medical Technologist'),
...
const ACTION_LABELS: Record<string, string> = {
  ...
  // ความปลอดภัย — แผนที่ห้องปฏิบัติการ / ห้องสารเคมี
  ...
  'lab_map.safety_asset.create':                  'เพิ่มอุปกรณ์ความปลอดภัย',
