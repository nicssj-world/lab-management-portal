...
**Goal:** ให้ผู้ดูแลลบ chemical holding จากหน้าทะเบียนสารเคมีได้จริง โดยลบ SDS metadata ที่ไม่ถูกอ้างอิงแล้วใน transaction เดียว, เก็บ SDS metadata/file ที่รายการอื่นใช้ร่วม, และลบ R2 object เฉพาะไฟล์ที่ไม่มี reference อื่น
...
- shared SDS version/publication/link ต้องตัดเฉพาะความสัมพันธ์ของ holding เป้าหมาย และต้องไม่มี partial delete
...
- [x] Write pure-planner tests for: room SDS/publication deletion, department SDS link deletion, shared SDS version retention with target publication deletion, same file referenced by another version being kept, orphan file being selected for cleanup, product master/other holding preservation.
- [x] Write route contract tests that require a `GET` impact preflight and a `DELETE` RPC path, delegate shared-reference handling to the cascade RPC, and never create a `holding_delete` change request for direct deletion.
...
- [x] Mark a dependency as shared only when another holding owns or publishes/links the same SDS version. Shared references are informational; they retain metadata instead of blocking the target holding delete. Do not mark a shared file binary by itself as a blocking dependency.
...
- [x] Lock the target holding and relevant dependency rows before calculating shared references. Detach a target-owned version that is still used elsewhere; delete only target-owned/legacy versions with no remaining references.
...
- [x] Add migration contract assertions for delete order, shared-reference retention/detachment, file reference checks, product preservation, and service-role permissions.
...
- [x] Implement `DELETE` by reloading the impact and calling `delete_chemical_holding_cascade` with the authenticated actor ID; shared references are handled atomically by the RPC.
- [x] Keep stale/not-found handling mapped without exposing raw database errors. The server must never create a pending change request for this workflow.
...
- [x] When shared dependencies exist, keep the destructive action enabled and show which SDS/version/reference is retained for the other holding.
...
- [x] Add stable client-facing handling for retained shared SDS references, holding not found, stale/preflight mismatch, and cleanup warning without exposing SQL text.
