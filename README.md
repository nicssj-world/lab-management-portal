...
- **Activity log labels** — the raw `audit_log.action` code (e.g. `outlab.certificate.create`) is translated to a Thai display name in **four separate places**, and all four drift independently, so a new module or a new audit action needs an entry in each:
  ...
  `npx tsx scripts/activity-log-labels.test.ts` statically scans every `app/api/**` and `lib/**` file for `audit_log` action codes (literals, ternaries, and known audit-helper wrappers like `auditRisk`/`auditIt`/`auditExternalQuality`/`auditChild`) and fails with the exact missing action + file if any of the four places above is out of sync — run it after adding a new audit action, and it should also be added to CI once one exists.
