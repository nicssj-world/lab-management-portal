-- Prevent two active Quality meetings from occupying the same date/time slot.
-- The application checks first for a friendly message; this trigger closes the
-- race where two users save the same slot at nearly the same time.
...
BEGIN
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION USING
      ERRCODE = '23P01',
      MESSAGE = 'ช่วงเวลาดังกล่าวมีประชุมแล้ว';
  END IF;
...
BEFORE INSERT OR UPDATE OF template_id, schedule_id, period_start, period_end,
  planned_date, planned_start_time, planned_end_time, note
ON public.quality_task_instances
FOR EACH ROW
