...
| ความปลอดภัย → แผนที่ห้องปฏิบัติการ | — (ทุกคนที่ล็อกอิน; ไม่มี resource gate) | `/staff/lab-map`, `/staff/lab-map/safety-assets`, `/staff/lab-map/print`; visitor popup inside `/v/[token]` | `/api/admin/lab-map/releases/*` |
| ความปลอดภัย → ห้องสารเคมี / SDS | — (Admin/Manager/safety editor หรือผู้ได้รับมอบหมายตาม `chemical_role_scopes`) | `/staff/lab-map/chemicals?view=registry\|layout\|sds-chemicals\|sds-departments` (ค่าที่ถูกต้องอยู่ใน `CHEMICAL_HUB_VIEWS` ของ `lib/navigation.ts`), `/staff/lab-map/sds?view=chemicals\|departments` (redirect ไป `sds-chemicals`/`sds-departments`); public `/sds` (ไม่ต้องล็อกอิน) | `/api/admin/chemical-safety/*`, public `/api/public/sds/*`, `/api/public/department-sds/*`, `/api/public/safety-manual/MN-LAB-02` |
