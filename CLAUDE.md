...
The visitor map is reachable **only** from `ActiveVisitCard` as an accessible popup (`components/lab-map/VisitorMapDialog.tsx`): dialog role, Escape, focus trap, focus return, scroll lock. It does not change the URL, so the active visit and its self-checkout button survive. The standalone `/lab-map/[stationCode]` route, its Public Nav entry, and the public-home shortcut are removed and must stay removed (`scripts/lab-map-navigation.test.ts` asserts this).
