...
// หน่วยนับจำนวนอิสระ (ไม่ใช่ mL/L/g/kg) เช่น 'test' kit — ยอมรับได้ ไม่มีการแปลง/รวมหน่วย
assert.deepEqual(calculateHoldingTotalFromFields({ packageValue: 500, packageUnit: 'unknown', currentContainerCount: 7 }), { value: 3500, unit: 'unknown' })
...
assert.deepEqual(calculateHoldingTotal([{ value: 2, unit: 'test', count: 3 }]), { value: 6, unit: 'test' })
assert.deepEqual(calculateHoldingTotalFromFields({ packageValue: 25, packageUnit: 'test', currentContainerCount: 2 }), { value: 50, unit: 'test' })
assert.throws(() => calculateHoldingTotal([{ value: 1, unit: 'test', count: 1 }, { value: 1, unit: 'kit', count: 1 }]), /Cannot combine/i)
