...
import type { QualityTaskActionItem } from '@/lib/quality-tasks/types'
...
export function EvacuationClient({ initialDashboard, canEdit, canManage }: { initialDashboard: EvacuationDashboardDTO; canEdit: boolean; canManage: boolean }) {
  ...
  const [capaItems, setCapaItems] = useState<QualityTaskActionItem[]>([])
  ...
  useEffect(() => {
    const instanceId = selectedCycle?.taskInstanceId
    ...
    setCapaLoading(true)
    void fetch(`/api/admin/safety-tasks/occurrences/${instanceId}/action-items?sourceType=evacuation_drill_session&sourceId=${encodeURIComponent(sourceId)}`, { signal: controller.signal })
      .then(async response => {
        const body = await response.json().catch(() => ({})) as { items?: QualityTaskActionItem[]; error?: string }
        if (!response.ok) throw new Error(body.error ?? 'ไม่สามารถโหลด CAPA ของผลซ้อมได้')
        return body.items ?? []
      })
      .then(items => { if (!controller.signal.aborted) setCapaItems(items) })
      .catch(() => { if (!controller.signal.aborted) setCapaItems([]) })
      .finally(() => { if (!controller.signal.aborted) setCapaLoading(false) })
    return () => controller.abort()
  }, [selectedCycle?.taskInstanceId, selectedSession?.id])
  ...
  async function addCapa() {
    if (!selectedCycle?.taskInstanceId || !selectedSession || !capaText.trim()) return
    await run(async () => {
      const item = await jsonRequest<QualityTaskActionItem>(`/api/admin/safety-tasks/occurrences/${selectedCycle.taskInstanceId}/action-items`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          assignee: { userId: null, manualName: 'ผู้รับผิดชอบตามแผนซ้อม' },
          description: capaText.trim(), dueDate: capaDue || null,
          sourceType: 'evacuation_drill_session', sourceId: `${selectedSession.id}:${crypto.randomUUID()}`,
        }),
      })
      setCapaItems(items => [...items, item])
      setCapaText('')
      setCapaDue('')
    }, 'สร้าง CAPA/Action Item จากผลการซ้อมแล้ว')
  }

  async function toggleCapa(item: QualityTaskActionItem) {
    if (!selectedCycle?.taskInstanceId) return
    await run(async () => {
      const updated = await jsonRequest<QualityTaskActionItem>(`/api/admin/safety-tasks/occurrences/${selectedCycle.taskInstanceId}/action-items/${item.id}`, {
        method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ done: !item.doneAt }),
      })
      setCapaItems(items => items.map(current => current.id === updated.id ? updated : current))
    })
  }
...
function DrillsPanel({ dashboard, canEdit, busy, selectedCycle, selectedSession, cycleEditorId, cycleDraft, sessionDraft, setCycleEditorId, setCycleDraft, setSessionEditorId, setSessionDraft, onSaveCycle, onSaveSession, onLinkEvidence, capaItems, capaText, capaDue, capaLoading, setCapaText, setCapaDue, onAddCapa, onToggleCapa }: {
  dashboard: EvacuationDashboardDTO
  canEdit: boolean
  busy: boolean
  ...
  capaItems: QualityTaskActionItem[]
  capaText: string
  capaDue: string
  capaLoading: boolean
  setCapaText: (value: string) => void
  setCapaDue: (value: string) => void
  onAddCapa: () => Promise<void>
  onToggleCapa: (item: QualityTaskActionItem) => Promise<void>
}) {
...
function ActionItemsPanel({ cycle, session, canEdit, busy, items, loading, text, dueDate, onText, onDueDate, onAdd, onToggle }: {
  cycle: EvacuationDrillCycleDTO
  session: EvacuationDrillSessionDTO | null
  canEdit: boolean
  busy: boolean
  items: QualityTaskActionItem[]
  loading: boolean
  text: string
  dueDate: string
  ...
  onAdd: () => Promise<void>
  onToggle: (item: QualityTaskActionItem) => Promise<void>
}) {
  return <article className="evac-card" data-task-instance={cycle.taskInstanceId}>
    <div className="evac-card-head"><div><h2>CAPA / Action Item จากผลการซ้อม</h2><p>ข้อเบี่ยงเบนของ session นี้เชื่อมกลับงานและหลักฐานเดิม ไม่สร้างระบบ CAPA ซ้ำ</p></div>{session ? <Link className="evac-link" href="/staff/safety">เปิดงานและหลักฐาน</Link> : null}</div>
    {!session ? <div className="evac-empty"><p>เลือกผลการซ้อมก่อน เพื่อสร้างและติดตาม CAPA ให้ตรงกับเหตุการณ์</p></div> : loading ? <div className="evac-skeleton" role="status">กำลังโหลด CAPA…</div> : <>
      <div className="evac-list">{items.map(item => <label className="evac-list-item" key={item.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}><input type="checkbox" checked={Boolean(item.doneAt)} disabled={!canEdit || busy} onChange={() => void onToggle(item)} style={{ width: 18, minHeight: 18, marginTop: 3 }} /><span><strong style={{ textDecoration: item.doneAt ? 'line-through' : undefined }}>{item.description}</strong><small>{item.assignee.manualName ?? 'ยังไม่ระบุผู้รับผิดชอบ'} · {item.dueDate ? `ครบกำหนด ${formatDate(item.dueDate)}` : 'ไม่ระบุกำหนด'}{item.sourceId ? ` · source ${item.sourceId.split(':')[0]}` : ''}</small></span></label>)}{!items.length ? <div className="evac-empty"><p>ยังไม่มี CAPA จากผลการซ้อมนี้</p></div> : null}</div>
      {canEdit ? <div className="evac-form-grid"><label>การแก้ไข/ป้องกัน<input value={text} onChange={event => onText(event.target.value)} placeholder="ระบุสิ่งที่ต้องแก้ไข ป้องกัน หรือติดตาม" /></label><label>กำหนดเสร็จ<input type="date" value={dueDate} onChange={event => onDueDate(event.target.value)} /></label><Button size="lg" icon="plus" disabled={busy || !text.trim()} onClick={() => void onAdd()}>สร้าง CAPA จากข้อเบี่ยงเบน</Button></div> : null}
    </>}
  </article>
}
