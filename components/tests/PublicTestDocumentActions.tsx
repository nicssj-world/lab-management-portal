...
export function PublicTestDocumentActions({ testId, source, documentId, accessMode }: Props) {
  const [loading, setLoading] = useState<DocumentAction | null>(null)

  async function open(action: DocumentAction) {
    setLoading(action)
    try {
      const response = await fetch(`/api/tests/${testId}/document-actions/${source}/${documentId}?action=${action}`)
  ...
  const buttonStyle: React.CSSProperties = { width: 28, height: 28, borderRadius: 6, border: '1px solid var(--border)', background: 'transparent', cursor: 'pointer', color: '#2563EB', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }
