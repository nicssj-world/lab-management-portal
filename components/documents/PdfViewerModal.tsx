...
export function PdfViewerModal({ url, pdfJsUrl, title, mimeType = 'application/pdf', forcePdfJs = false, onClose }: { url: string; pdfJsUrl?: string | null; title: string; mimeType?: string | null; forcePdfJs?: boolean; onClose: () => void }) {
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.75)', zIndex: 2000, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, minHeight: 0 }}>
        <PdfViewer url={url} pdfJsUrl={pdfJsUrl} fileName={title} mimeType={mimeType} forcePdfJs={forcePdfJs} />
      </div>
    </div>
  )
}
