...
type FormData = {
  point: MonthlySafetyPoint; template: TemplateSnapshot
  inspection: Record<string, unknown> | null
  skip: { reason: string; at: string } | null
}
type SubmittedAnswer = { supplyId: string; result?: SpillKitItemResult; note?: string | null; clarity?: string; bottleCondition?: string; correctiveAction?: string | null }
type SubmittedRow = { key: string; label: string; value: string; note: string | null; issue: boolean }
...
const EDITABLE_STATUSES: SafetyPointStatus[] = ['pending', 'due_soon', 'overdue']
const RESULT_LABEL: Record<string, string> = Object.fromEntries(RESULT_OPTIONS.map(option => [option.value, option.label]))

// จุดตรวจที่ส่งผลหรือถูกข้ามแล้วต้องเปิดดูย้อนหลังได้ ไม่ใช่ปิดตายจนต้องไปเปิด PDF
function submittedRows(form: FormData): SubmittedRow[] {
  const submission = (form.inspection?.form_snapshot as { submission?: { kind?: string; answers?: SubmittedAnswer[]; bottles?: SubmittedAnswer[] } } | undefined)?.submission
  if (!submission) return []
  const supplies = form.template.supplies ?? []
  const labelById = new Map(supplies.map(supply => [supply.id, supply.labelTh || supply.internalCode]))
  if (submission.kind === 'nss') {
    return (submission.bottles ?? []).map((bottle, index) => {
      const issue = bottle.clarity === 'turbid' || bottle.bottleCondition === 'cracked'
      return {
        key: bottle.supplyId || `bottle-${index}`,
        label: labelById.get(bottle.supplyId) ?? `ขวดที่ ${index + 1}`,
        value: `${bottle.clarity === 'turbid' ? 'ขุ่น' : 'ใส'} · ${bottle.bottleCondition === 'cracked' ? 'มีรอยร้าว' : 'สมบูรณ์'}`,
        note: bottle.correctiveAction || null, issue,
      }
    })
  }
  return (submission.answers ?? []).map((answer, index) => ({
    key: answer.supplyId || `answer-${index}`,
    label: labelById.get(answer.supplyId) ?? answer.supplyId,
    value: RESULT_LABEL[answer.result ?? ''] ?? answer.result ?? '—',
    note: answer.note || null,
    issue: !['normal', 'na'].includes(answer.result ?? ''),
  }))
}
...
export function MonthlySafetyInspectionBoard({ isEditor, fiscalYear }: { isEditor: boolean; fiscalYear: number }) {
  ...
  useEffect(() => {
    if (!selected || !EDITABLE_STATUSES.includes(selected.point.status)) return
  ...
  return <section className="msb">
    ...
    <div className="msb-list" aria-busy={loading}>
      {loading ? <div className="msb-empty">กำลังเตรียมรอบตรวจประจำเดือน…</div> : visible.map(point => <button key={point.roundItemId} className="msb-row" onClick={() => void openPoint(point)}>
    ...
    {selected && <div className="msb-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) setSelected(null) }}>
      <section className="msb-dialog" role="dialog" aria-modal="true" aria-labelledby="msb-form-title">
        ...
        <div className="msb-form-body">
          {!EDITABLE_STATUSES.includes(selected.point.status) ? <div className="msb-readonly">
            <p className="msb-readonly-head">
              {selected.skip
                ? `ข้ามจุดตรวจนี้เมื่อ ${thaiDate(selected.skip.at)} · เหตุผล: ${selected.skip.reason || '—'}`
                : `ส่งผลตรวจเมื่อ ${thaiDate(selected.point.submittedAt)} โดย ${selected.point.submittedByName ?? 'ไม่ระบุชื่อ'} · พบปัญหา ${selected.point.issueCount} รายการ`}
            </p>
            {submittedRows(selected).map(row => <article key={row.key} className={row.issue ? 'is-issue' : ''}>
              <b>{row.label}</b><span>{row.value}</span>{row.note && <small>{row.note}</small>}
            </article>)}
            {!selected.skip && !submittedRows(selected).length && <p className="msb-readonly-empty">ไม่พบรายละเอียดผลตรวจที่บันทึกไว้ ดูสรุปได้จาก PDF ของเดือนนี้</p>}
          </div> : selected.point.profile !== 'nss_eyewash' ? <>
        ...
        {EDITABLE_STATUSES.includes(selected.point.status)
          ...
          : <footer><span>บันทึกนี้ปิดแล้ว แก้ไขไม่ได้</span><a className="msb-download" href={`/api/admin/safety-tasks/monthly-inspections/report?fiscalYear=${fiscalYear}&roundItemId=${selected.point.roundItemId}`}><Icon name="download" size={15} />PDF จุดตรวจนี้</a></footer>}
      </section>
    </div>}
    <style jsx>{`
      ...
      .msb-readonly{display:flex;flex-direction:column;gap:8px}.msb-readonly-head{margin:0;padding:10px;border-radius:9px;background:#f0fdfa;color:#115e59;font-size:11px}.msb-readonly article{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:4px 10px;padding:10px 11px;border:1px solid var(--border);border-radius:9px}.msb-readonly article.is-issue{border-color:#fdba74;background:#fff7ed}.msb-readonly article b{font-size:11px}.msb-readonly article span{font-size:11px;font-weight:800}.msb-readonly article small{grid-column:1/-1;color:var(--muted);font-size:10px}.msb-readonly-empty{margin:0;color:var(--muted);font-size:11px}
