...
export function QualityTaskDashboard({ actorId, level, initialMonth, initialOccurrences, templates, people }: Props) {
  ...
  const filtered=useMemo(()=>items.filter(o=>(!category||o.template.categoryCode===category)&&(!state||o.urgency===state)&&(!owner||o.template.ownerText===owner)&&(!assignee||o.assignees.some(e=>e.userId===assignee))&&(!search||`${o.template.title} ${o.template.ownerText}`.toLowerCase().includes(search.toLowerCase()))),[items,category,state,owner,assignee,search])
  ...
  return <div style={{display:'grid',gap:18}}>
    ...
    <section><h2 style={{fontSize:16,margin:'0 0 10px'}}>กิจกรรมทั้งหมด ({filtered.length})</h2><div className="qt-desktop" style={{overflow:'auto',border:'1px solid var(--border)',borderRadius:12,background:'var(--card)'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}><thead><tr>{['กิจกรรม','งวด','วันนัด/เส้นตาย','ผู้รับผิดชอบ','สถานะ'].map(h=><th key={h} style={th}>{h}</th>)}</tr></thead><tbody>{filtered.map(o=><tr key={o.key} onClick={()=>setSelected(o)} style={{cursor:'pointer',borderTop:'1px solid var(--border)'}}><td style={td}><b>{o.template.title}</b><div style={{fontSize:11,color:'var(--muted)'}}>หมวด {o.template.categoryCode} · {o.template.ownerText}</div></td><td style={td}>{o.periodLabel}</td><td style={td}>{fmt(o.plannedDate??o.effectiveDueDate)}{!o.plannedDate&&<div style={{fontSize:10.5,color:'#D97706'}}>ยังไม่กำหนดวัน</div>}</td><td style={td}>{o.assignees.map(e=>assigneeName(e,people)).filter(Boolean).join(', ')||'—'}</td><td style={td}><Status o={o}/></td></tr>)}</tbody></table></div><div className="qt-mobile">{filtered.map(o=><button key={o.key} onClick={()=>setSelected(o)} style={{background:'var(--card)',border:'1px solid var(--border)',borderRadius:12,padding:13,textAlign:'left',fontFamily:'inherit',color:'var(--ink)'}}><b>{o.template.title}</b><div style={{fontSize:11,color:'var(--muted)',margin:'5px 0'}}>หมวด {o.template.categoryCode} · {o.periodLabel} · {fmt(o.plannedDate??o.effectiveDueDate)}</div><Status o={o}/></button>)}</div></section>
    {selected&&<div onClick={()=>setSelected(null)} style={overlay}><div onClick={e=>e.stopPropagation()} style={modal}><div style={{display:'flex',gap:10,justifyContent:'space-between'}}><div><div style={{fontSize:11,color:'var(--primary)',fontWeight:800}}>หมวด {selected.template.categoryCode} · {selected.periodLabel}</div><h2 style={{margin:'5px 0 0',fontSize:20}}>{selected.template.title}</h2></div><button onClick={()=>setSelected(null)} style={closeStyle}>×</button></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginTop:14}}><Info label="ทีม/บทบาท" value={selected.template.ownerText}/><Info label="ผู้รับผิดชอบ" value={selected.assignees.map(e=>assigneeName(e,people)).filter(Boolean).join(', ')||'ยังไม่มอบหมาย'}/><Info label="ความถี่" value={selected.template.frequencyText}/><Info label="วันนัด" value={fmt(selected.plannedDate)}/><Info label="เส้นตาย" value={fmt(selected.effectiveDueDate)}/><Info label="ผู้เข้าร่วมประชุม" value={selected.participants.map(p=>p.name).join(', ')||'ยังไม่กำหนด'}/></div>{selected.participants.length>0&&<div style={{marginTop:10}}><Button variant="secondary" size="sm" icon="download" onClick={downloadSignInSheet}>ดาวน์โหลด PDF ใบลงนาม ({selected.participants.length} คน)</Button></div>}{selected.plannedDate&&(selected.plannedDate<selected.periodStart||selected.plannedDate>selected.periodEnd)&&<div style={{marginTop:9,padding:8,borderRadius:8,background:'#FEF3C7',color:'#92400E',fontSize:11}}>วันนัดอยู่นอกช่วงงวดเดิม แต่ระบบยังนับเป็น {selected.periodLabel}</div>}<div style={{marginTop:12}}><Status o={selected}/></div>{error&&<div style={{marginTop:10,color:'#DC2626',fontSize:12}}>{error}</div>}{canAct&&<div style={{display:'grid',gap:8,marginTop:14,paddingTop:14,borderTop:'1px solid var(--border)'}}><label style={labelStyle}>กำหนดวัน<input type="date" defaultValue={selected.plannedDate??''} onChange={e=>mutate(selected,{action:'schedule',plannedDate:e.target.value||null})} disabled={busy} style={inputStyle}/></label><Button variant="secondary" size="sm" onClick={()=>{const note=prompt('หมายเหตุ',selected.note??'');if(note!==null)mutate(selected,{action:'schedule',plannedDate:selected.plannedDate,note})}}>บันทึกหมายเหตุ</Button>{level==='edit'&&<label style={labelStyle}>ผู้รับผิดชอบงวดนี้<AssigneeListEditor entries={selected.assignees} onChange={entries=>mutate(selected,{action:'schedule',plannedDate:selected.plannedDate,assignees:entries})} people={people}/></label>}{level==='edit'&&<label style={labelStyle}>ผู้เข้าร่วมประชุม (เฉพาะงวดนี้)<div style={{display:'flex',alignItems:'center',gap:9,flexWrap:'wrap'}}><span style={{fontSize:12,color:'var(--ink)',fontWeight:500}}>{selected.participantDepts.length===0&&selected.participantUserIds.length===0?'ใช้ค่าเริ่มต้นของกิจกรรม':`${selected.participantDepts.length} แผนก · ${selected.participantUserIds.length} คน (เฉพาะงวดนี้)`}</span><Button variant="secondary" size="sm" onClick={()=>setParticipantModalOpen(true)}>กำหนดผู้เข้าร่วม</Button></div></label>}<div><b style={{fontSize:12}}>PDF หลักฐาน {selected.template.evidenceRequired?'(บังคับ)':'(ไม่บังคับ)'}</b><div style={{display:'grid',gap:5,marginTop:6}}>{selected.attachments.map(a=><div key={a.id} style={{display:'flex',gap:6,alignItems:'center'}}><a href={`/api/admin/quality-tasks/attachments/${a.id}`} target="_blank" style={{fontSize:12,color:'var(--primary)',flex:1}}>📎 {a.fileName}</a>{level==='edit'&&<button onClick={()=>removeAttachment(a.id)} style={{...closeStyle,width:26,height:26,fontSize:13}}>ลบ</button>}</div>)}</div><Button variant="secondary" size="sm" onClick={()=>fileRef.current?.click()} disabled={busy} style={{marginTop:7}}>แนบ PDF</Button><input ref={fileRef} type="file" accept="application/pdf,.pdf" hidden onChange={e=>{const f=e.target.files?.[0];if(f)upload(f)}}/></div><div style={{display:'flex',gap:8,justifyContent:'flex-end'}}>{selected.status==='open'?<Button variant="primary" disabled={busy} onClick={()=>mutate(selected,{action:'complete'})}>ทำแล้ว</Button>:level==='edit'?<Button variant="secondary" disabled={busy} onClick={()=>{const reason=prompt('เหตุผลที่เปิดงานใหม่');if(reason)mutate(selected,{action:'reopen',reason})}}>เปิดงานใหม่</Button>:null}</div></div>}<div style={{marginTop:16,borderTop:'1px solid var(--border)',paddingTop:12}}><b style={{fontSize:12}}>ประวัติกิจกรรม</b>{history.length?<div style={{display:'grid',gap:7,marginTop:7}}>{history.map(h=><div key={h.id} style={{fontSize:11,color:'var(--muted)'}}><b style={{color:'var(--ink)'}}>{h.actor_name??'System'}</b> · {h.action} · {new Date(h.created_at).toLocaleString('th-TH',{timeZone:'Asia/Bangkok'})}</div>)}</div>:<div style={{fontSize:11,color:'var(--muted)',marginTop:6}}>ยังไม่มีประวัติที่บันทึก</div>}</div></div></div>}
