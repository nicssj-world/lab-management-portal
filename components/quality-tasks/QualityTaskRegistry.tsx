...
function blank(no: number): QualityTaskTemplate {
  return {
    id: "",
    sourceKey: null,
    categoryCode: "A",
...
export function QualityTaskRegistry({
  level,
  initialTemplates,
  people,
}: {
  level: PermLevel;
  initialTemplates: QualityTaskTemplate[];
  people: Person[];
}) {
  ...
  const filtered = useMemo(
    () =>
      items.filter(
        (t) =>
          (!cat || t.categoryCode === cat) &&
          (!search ||
            `${t.title} ${t.ownerText} ${t.referenceCode ?? ""}`
              .toLowerCase()
  ...
  async function refresh() {
    const r = await fetch("/api/admin/quality-tasks/templates");
  ...
  async function save() {
    ...
    try {
      const url = draft.id
        ? `/api/admin/quality-tasks/templates/${draft.id}`
        : "/api/admin/quality-tasks/templates";
      const r = await fetch(url, {
        method: draft.id ? "PATCH" : "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...draft,
          schedules: draft.schedules.map((s) => ({
            ...s,
            templateId: draft.id,
          })),
        }),
  ...
  async function remove(t: QualityTaskTemplate) {
    if (!confirm(`ลบกิจกรรม "${t.title}"?`)) return;
    setBusy(true);
    setError("");
    try {
  ...
  function set<K extends keyof QualityTaskTemplate>(
    key: K,
    value: QualityTaskTemplate[K],
  ) {
    setDraft((d) => (d ? { ...d, [key]: value } : d));
  }
  function updateSchedule(index: number, patch: Partial<QualityTaskSchedule>) {
    setDraft((d) =>
      d
        ? {
            ...d,
            schedules: d.schedules.map((s, i) =>
              i === index ? { ...s, ...patch } : s,
            ),
          }
        : d,
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      <PageHeader
        eyebrow="Quality Task Registry"
        title="ทะเบียนกิจกรรมคุณภาพ"
        subtitle={`${items.length} กิจกรรม · 9 หมวด · อ้างอิง ISO 15189:2022`}
        actions={
          <>
            <Link href="/staff/quality-tasks">
            ...
            {level === "edit" && (
              <Button
                ...
                onClick={() =>
                  setDraft(
                    blank(
                      Math.max(0, ...items.map((x) => x.activityNo ?? 0)) + 1,
                    ),
                  )
                }
              >
          ...
          <thead>
            <tr>
              {[
                ...
                <th key={h} style={h === "กิจกรรม" ? th : thCenter}>
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {filtered.map((t) => (
              ...
              >
                ...
                <td style={tdCenter}>
                  <Badge text={t.frequencyText} />
                </td>
                <td style={tdCenter}>
                  {t.referenceCode ? (
                    <Badge text={t.referenceCode} />
                  ) : (
                ...
                <td style={tdCenter}>
                  <Badge
                    text={t.evidenceRequired ? "บังคับ PDF" : "PDF ไม่บังคับ"}
                    color={t.evidenceRequired ? "#D97706" : "#64748B"}
                  />
                </td>
                {level === "edit" && (
                  <td style={tdCenter}>
            ...
            >
                ...
                <select
                  ...
                  onChange={(e) =>
                    setDraft({
                      ...
                      categoryName: CATS[e.target.value],
                    })
                  }
                  style={inputStyle}
                >
              ...
              <Field label="รายละเอียด" wide>
                <textarea
                  ...
                  style={{ ...inputStyle, height: 70, padding: 9 }}
                />
              </Field>
              <Field label="เลขเอกสาร">
                <input
                  value={draft.referenceCode ?? ""}
                  onChange={(e) => set("referenceCode", e.target.value || null)}
                  style={inputStyle}
                />
              </Field>
              <Field label="ความถี่เดิม">
                <input
                  value={draft.frequencyText}
                  onChange={(e) => set("frequencyText", e.target.value)}
                  style={inputStyle}
                />
              </Field>
              <Field label="ทีม/บทบาท" wide>
                <input
              ...
              <Field label="ประเภท">
                <select
                  ...
                  onChange={(e) =>
                    set("taskKind", e.target.value as "activity" | "meeting")
                  }
                  style={inputStyle}
                >
              ...
              <Field label="ผู้รับผิดชอบเริ่มต้น" wide>
                <AssigneeListEditor
                  entries={draft.defaultAssignees}
                  onChange={(entries) => set("defaultAssignees", entries)}
                  people={people}
                />
              </Field>
              <Field label="ผู้เข้าร่วมประชุม (ค่าเริ่มต้น)" wide>
                <div
                  style={{
                    display: "flex",
                    alignItems: "center",
                ...
                >
                  <span style={{ fontSize: 12, color: "var(--muted)" }}>
                    {draft.defaultParticipantDepts.length === 0 &&
                    draft.defaultParticipantUserIds.length === 0
                      ? "ยังไม่กำหนด"
                      : `${draft.defaultParticipantDepts.length} แผนก · ${draft.defaultParticipantUserIds.length} คน`}
                  </span>
                  <Button
                    variant="secondary"
                    size="sm"
              ...
              {draft.schedules.map((s, i) => (
                ...
                >
                  ...
                  <input
                    ...
                    value={s.startsOn}
                    onChange={(e) =>
                      updateSchedule(i, { startsOn: e.target.value })
                    }
                    style={inputStyle}
                  />
                  <Button
                    ...
                    onClick={() =>
                      set(
                        "schedules",
                        draft.schedules.filter((_, x) => x !== i),
                      )
                    }
                  >
                    ลบ
      ...
      {participantModalOpen && draft && (
        <ParticipantAudienceModal
          depts={draft.defaultParticipantDepts}
          userIds={draft.defaultParticipantUserIds}
          people={people}
          ...
          onSave={(depts, userIds) => {
            setDraft((d) =>
              d
                ? {
                    ...d,
                    defaultParticipantDepts: depts,
                    defaultParticipantUserIds: userIds,
                  }
                : d,
            );
