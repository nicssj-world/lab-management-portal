...
const LIFECYCLE_META: Record<string, { label: string; fg: string; bg: string }> = {
  current: { label: 'ใช้งาน', fg: 'var(--success)', bg: 'rgba(22,163,74,.12)' },
  superseded: { label: 'ถูกแทนที่', fg: 'var(--warning)', bg: 'rgba(217,119,6,.12)' },
  revoked: { label: 'เพิกถอน', fg: 'var(--danger)', bg: 'rgba(220,38,38,.12)' },
}

function LifecycleBadge({ lifecycle }: { lifecycle: string }) {
  const meta = LIFECYCLE_META[lifecycle] ?? { label: lifecycle, fg: 'var(--muted)', bg: 'var(--surface-2)' }
  return <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: 999, fontSize: 11.5, fontWeight: 700, color: meta.fg, background: meta.bg, lineHeight: 1.4 }}>
    <span style={{ width: 6, height: 6, borderRadius: '50%', background: meta.fg, flexShrink: 0 }} />
    {meta.label}
  </span>
}
...
export function OutlabDashboard({ overview, canEdit, isAdmin, activeSection }: { overview: OutlabOverview; canEdit: boolean; isAdmin: boolean; activeSection: OutlabSection }) {
  ...
  async function purgeCertificate(cert: OutlabCertificate) {
    if (!confirm(`ลบใบรับรอง "${cert.standard_name}" ถาวร ใช่หรือไม่? การลบนี้ไม่สามารถกู้คืนได้`)) return
    try { setError(''); await jsonRequest(`/api/admin/outlab/certificates/${cert.id}/purge`, 'DELETE', {}); refresh() } catch (cause) { setError(cause instanceof Error ? cause.message : 'ลบไม่สำเร็จ') }
  }
