...
type Tab = 'dashboard' | 'laboratories' | 'services' | 'certificates' | 'settings'
const tabs: Array<[Tab, string]> = [['dashboard', 'ภาพรวม'], ['laboratories', 'ห้องปฏิบัติการ'], ['services', 'บริการส่งต่อ'], ['certificates', 'ใบรับรอง']]
const tabIcons: Record<Tab, string> = { dashboard: 'dash', laboratories: 'building', services: 'clipboard', certificates: 'doc', settings: 'settings' }
...
export function OutlabDashboard({ overview, canEdit, isAdmin }: { overview: OutlabOverview; canEdit: boolean; isAdmin: boolean }) {
  ...
  return <div className="eq-wrap quality-module">
    ...
    <PageHeader eyebrow="EXTERNAL LABORATORY" title="ห้องปฏิบัติการภายนอก" subtitle="บริการส่งต่อ มาตรฐาน และใบรับรองของห้องปฏิบัติการภายนอก" marginBottom={0} actions={<div className="quality-module-actions">{isAdmin && <button type="button" style={{ ...button, background: '#475569' }} onClick={() => setTab('settings')}>จัดการระบบ</button>}<a style={{ ...button, display: 'inline-flex', alignItems: 'center', textDecoration: 'none', background: '#15803D' }} href="/api/admin/outlab/export">Excel</a><a style={{ ...button, display: 'inline-flex', alignItems: 'center', textDecoration: 'none', background: '#B91C1C' }} href="/api/admin/outlab/export?format=pdf">PDF</a></div>} />
    <div role="tablist" aria-label="เมนูห้องปฏิบัติการภายนอก" className="eq-tabs">{tabs.map(([key, label]) => <button key={key} type="button" role="tab" aria-selected={tab === key} aria-controls={`outlab-panel-${key}`} className={`eq-tab ${tab === key ? 'on' : ''}`} onClick={() => setTab(key)}><Icon name={tabIcons[key]} size={15} />{label}</button>)}</div>
    {error && <div role="alert" style={{ padding: 10, borderRadius: 8, background: '#FEF2F2', color: '#B91C1C' }}>{error}</div>}
    {tab === 'dashboard' && <>
      <div className="eq-grid">{[['ห้องที่ใช้งาน', overview.summary.laboratories], ['บริการที่เปิด', overview.summary.services], ['ใกล้หมดอายุ', overview.summary.expiring], ['หมดอายุ', overview.summary.expired], ['ไม่มีใบปัจจุบัน', overview.summary.missingCurrentCertificate]].map(([label, count]) => <div key={String(label)} style={card}><div style={{ color: 'var(--muted)', fontSize: 12 }}>{label}</div><div style={{ fontSize: 28, fontWeight: 800, marginTop: 4 }}>{count}</div></div>)}</div>
