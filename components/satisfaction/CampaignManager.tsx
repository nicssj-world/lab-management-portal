...
export function CampaignManager({ campaigns, surveys }: { campaigns: SatisfactionCampaignListItem[]; surveys: SatisfactionSurveyListItem[] }) {
  ...
  return (
    <div>
      <div style={{ padding: 16, borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}><div><h2 style={{ margin: 0, color: 'var(--ink)', fontSize: 15 }}>รอบเก็บข้อมูลและ QR Code</h2><p style={{ margin: '4px 0 0', color: 'var(--muted)', fontSize: 12 }}>สร้างรอบจากเวอร์ชันที่เผยแพร่แล้วเท่านั้น</p></div><Button size="sm" icon="plus" onClick={() => setCreating((value) => !value)}>เปิดรอบใหม่</Button></div>
      {creating && <div style={{ padding: 16, borderBottom: '1px solid var(--border)', background: 'var(--surface-2)' }}><div style={{ display: 'grid', gridTemplateColumns: 'minmax(180px,1fr) minmax(180px,1fr)', gap: 10 }}><label style={{ fontSize: 12, color: 'var(--muted)' }}>ชื่อรอบ<input className="builder-input" style={{ marginTop: 5 }} value={name} onChange={(event) => setName(event.target.value)} placeholder="เช่น รอบปีงบประมาณ 2569" /></label><label style={{ fontSize: 12, color: 'var(--muted)' }}>แบบสำรวจ<select className="builder-input" style={{ marginTop: 5 }} value={surveyId} onChange={(event) => setSurveyId(event.target.value)}><option value="">เลือกแบบสำรวจ</option>{published.map((survey) => <option key={survey.id} value={survey.id}>{survey.code} · V{survey.latestVersion}</option>)}</select></label><label style={{ fontSize: 12, color: 'var(--muted)' }}>จำนวนคำตอบสูงสุด (ไม่บังคับ)<input className="builder-input" inputMode="numeric" type="number" min={1} value={responseLimit} onChange={(event) => setResponseLimit(event.target.value)} /></label><label style={{ alignSelf: 'end', minHeight: 38, display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--ink)' }}><input type="checkbox" checked={onePerDevice} onChange={(event) => setOnePerDevice(event.target.checked)} /> จำกัดหนึ่งคำตอบต่ออุปกรณ์</label></div><div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 10 }}><Button onClick={create} disabled={busy}>{busy ? 'กำลังสร้าง…' : 'สร้างเป็นฉบับร่าง'}</Button></div></div>}
      ...
      {qr && <div style={{ position: 'fixed', inset: 0, zIndex: 110, background: 'rgba(15,23,42,.58)', display: 'grid', placeItems: 'center', padding: 16 }}><Card style={{ width: 'min(430px,100%)', textAlign: 'center' }}><h2 style={{ margin: 0, color: 'var(--ink)', fontSize: 18 }}>{qr.name}</h2><p style={{ color: 'var(--muted)', fontSize: 12 }}>สแกนเพื่อเปิดแบบสำรวจสาธารณะ</p><img src={qr.dataUrl} alt={`QR Code ${qr.name}`} style={{ width: 'min(300px,100%)', borderRadius: 12 }} /><div style={{ display: 'flex', gap: 8, justifyContent: 'center', marginTop: 12, flexWrap: 'wrap' }}><a href={qr.dataUrl} download={`${qr.name}-qr.png`} style={{ textDecoration: 'none' }}><Button icon="download">ดาวน์โหลด PNG</Button></a><Button variant="secondary" onClick={() => navigator.clipboard.writeText(qr.url)}>คัดลอกลิงก์</Button><Button variant="ghost" onClick={() => setQr(null)}>ปิด</Button></div><code style={{ display: 'block', marginTop: 12, padding: 8, borderRadius: 7, background: 'var(--surface-2)', color: 'var(--muted)', fontSize: 10, overflowWrap: 'anywhere' }}>{qr.url}</code></Card></div>}
    </div>
  )
}
