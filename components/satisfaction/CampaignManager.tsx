...
export function CampaignManager({ campaigns, surveys }: { campaigns: SatisfactionCampaignListItem[]; surveys: SatisfactionSurveyListItem[] }) {
  ...
  const remove = async (campaign: SatisfactionCampaignListItem) => {
    ...
    try {
      ...
      const result = await response.json(); if (!response.ok) throw new Error(result.error ?? 'ลบรอบไม่สำเร็จ')
      window.location.reload()
    } catch (caught) { setError(caught instanceof Error ? caught.message : 'ลบรอบไม่สำเร็จ') } finally { setBusy(false) }
  }
  ...
  return (
    <div className="campaign-manager">
      ...
      {campaigns.length === 0 ? <EmptyState title="ยังไม่มีรอบเก็บข้อมูล" hint="เผยแพร่แบบสำรวจ แล้วสร้างรอบเพื่อรับ QR Code" icon="calendar" /> : <div className="satisfaction-table-wrap"><table className="satisfaction-table"><thead><tr><th>รอบ / แบบ</th><th>สถานะ</th><th>คำตอบ</th><th>QR และการทำงาน</th></tr></thead><tbody>{campaigns.map((campaign) => <tr key={campaign.id}><td><strong>{campaign.name}</strong><div style={{ color: 'var(--muted)', marginTop: 3 }}>{campaign.surveyCode} · V{campaign.versionNumber}</div></td><td><Badge color={campaign.status === 'open' ? 'green' : campaign.status === 'draft' ? 'amber' : 'gray'} dot>{campaign.status === 'open' ? 'เปิดรับ' : campaign.status === 'draft' ? 'ฉบับร่าง' : 'ปิดแล้ว'}</Badge></td><td>{campaign.responseCount.toLocaleString('th-TH')}{campaign.responseLimit ? ` / ${campaign.responseLimit.toLocaleString('th-TH')}` : ''}</td><td><div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}><Button size="sm" variant="secondary" icon="download" onClick={() => showQr(campaign)}>QR</Button>{campaign.status === 'draft' && <Button size="sm" onClick={() => patch(campaign.id, { status: 'open' })} disabled={busy}>เปิดรับ</Button>}{campaign.status === 'open' && <Button size="sm" variant="danger" onClick={() => patch(campaign.id, { status: 'closed' })} disabled={busy}>ปิดรอบ</Button>}{campaign.responseCount === 0 && <Button size="sm" variant="ghost" icon="trash" onClick={() => remove(campaign)} disabled={busy}>ลบรอบ</Button>}</div></td></tr>)}</tbody></table></div>}
