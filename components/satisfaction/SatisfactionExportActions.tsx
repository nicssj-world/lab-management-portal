...
export function SatisfactionExportActions({ campaigns, actorRole }: { campaigns: SatisfactionCampaignListItem[]; actorRole: string }) {
  ...
  return <div className="satisfaction-overview-actions"><span className="satisfaction-visually-hidden" id="satisfaction-report-actions-label">การส่งออกรายงานและเผยแพร่ KPI</span><div className="satisfaction-export-controls" aria-labelledby="satisfaction-report-actions-label"><label className="satisfaction-export-field">รอบรายงาน<select aria-label="เลือกรอบสำหรับรายงาน" value={campaignId} onChange={(event) => setCampaignId(event.target.value)}>{campaigns.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="satisfaction-export-field">ปีงบประมาณ<input aria-label="ปีงบประมาณ" type="number" inputMode="numeric" value={fiscalYear} onChange={(event) => setFiscalYear(Number(event.target.value))} /></label><Button size="sm" variant="secondary" icon="download" onClick={excel} disabled={Boolean(busy)}>{busy === 'excel' ? 'กำลังสร้าง…' : 'Excel'}</Button><Button size="sm" variant="secondary" icon="doc" onClick={printPdf} disabled={Boolean(busy)}>{busy === 'pdf' ? 'กำลังสร้าง…' : 'พิมพ์ / PDF'}</Button>{canExportComments && <Button size="sm" variant="secondary" icon="inbox" onClick={exportComments} disabled={Boolean(busy)}>{busy === 'comments' ? 'กำลังสร้าง…' : 'ความคิดเห็น'}</Button>}{canEditKpi && campaign?.status === 'closed' && <Button size="sm" icon="chart" onClick={publishKpi} disabled={Boolean(busy)}>{busy === 'kpi' ? 'กำลังเผยแพร่…' : 'เผยแพร่ KPI'}</Button>}</div>{statusMessage && <div className="satisfaction-export-status" aria-live="polite">{statusMessage}</div>}{error && <div className="satisfaction-export-error" role="alert">{error}</div>}</div>
}
