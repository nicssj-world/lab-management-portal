    ...
    <div className="satisfaction-chart-grid">
      ...
      <Card className="satisfaction-chart-wide">
        ...
        <div className="chart-box" aria-label="กราฟแนวโน้มความพึงพอใจ">
          <ResponsiveContainer width="100%" height="100%"><LineChart data={data.trend} margin={{ top: 16, right: 28, left: -10, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" stroke="var(--border)" /><XAxis dataKey="period" tick={{ fontSize: 11, fill: 'var(--muted)' }} /><YAxis domain={[0, 100]} tick={{ fontSize: 11, fill: 'var(--muted)' }} /><Tooltip /><Line type="monotone" dataKey="normalizedPct" name="คะแนน" stroke="#0F766E" strokeWidth={2.5} dot={{ r: 4 }}><LabelList dataKey="normalizedPct" position="top" formatter={(value: unknown) => value === null ? '—' : `${value}%`} /></Line></LineChart></ResponsiveContainer>
        </div>
      ...
      <Card>
        ...
        <div className="chart-box" aria-label="กราฟคะแนนรายคำถาม"><ResponsiveContainer width="100%" height="100%"><BarChart data={questionData} layout="vertical" margin={{ top: 6, right: 38, left: 18, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" stroke="var(--border)" /><XAxis type="number" domain={[0, 100]} hide /><YAxis type="category" dataKey="name" width={130} tick={{ fontSize: 10, fill: 'var(--muted)' }} /><Tooltip /><Bar dataKey="value" name="คะแนน" fill="#2563EB" radius={[0, 5, 5, 0]}><LabelList dataKey="value" position="right" formatter={(value: unknown) => `${value}%`} /></Bar></BarChart></ResponsiveContainer></div>
      ...
      <Card>
        ...
        <div className="chart-box" aria-label="กราฟการกระจายระดับคะแนน"><ResponsiveContainer width="100%" height="100%"><BarChart data={distributionData} layout="vertical" margin={{ top: 30, right: 20, left: 5, bottom: 20 }}><XAxis type="number" hide /><YAxis type="category" dataKey="name" hide /><Tooltip /><Legend />{[1,2,3,4,5].map((score, index) => <Bar key={score} dataKey={`s${score}`} name={`ระดับ ${score}`} stackId="likert" fill={COLORS[index]}><LabelList dataKey={`s${score}`} position="center" fill="#fff" formatter={(value: unknown) => Number(value) > 0 ? String(value) : ''} /></Bar>)}</BarChart></ResponsiveContainer></div>
