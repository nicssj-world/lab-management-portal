...
export function ActiveVisitCard({ token, visit }: { token: string; visit: ActiveVisitorDTO }) {
  ...
  const mapHref = visit.checkpointCode
    ? `/lab-map/office?destination=${encodeURIComponent(visit.checkpointCode)}`
    : '/lab-map/office'
    ...
    <section className="active-visit-card">
      ...
      <div className="active-visit-destination">
        ...
        <div>
          ...
          <span>{visit.checkpointCode ? 'กรุณาหยุดและติดต่อเจ้าหน้าที่ ณ จุดสแกนนิ้วมือ' : 'กรุณาเลือกปลายทางบนแผนที่หรือติดต่อสำนักงาน'}</span>
        </div>
      </div>
      ...
      <div className="active-visit-links">
        <Link href={mapHref}><Icon name="building" size={16} /> เปิดแผนที่ไปจุดสแกน</Link>
        <Link href="/lab-map/office?mode=safety"><Icon name="shield" size={16} /> ดูทางออกและความปลอดภัย</Link>
      </div>
...
@media(prefers-reduced-motion:reduce){.active-visit-links a{transition:none}}
