...
const restoreMeetingMigrationName = readdirSync(join(process.cwd(), 'supabase/migrations'))
  .find(name => name.includes('restore_safety_meeting_to_quality') && name.endsWith('.sql'))
assert.ok(restoreMeetingMigrationName, 'safety meeting restoration migration exists')
const restoreMeetingSql = read(`supabase/migrations/${restoreMeetingMigrationName}`)
...
assert.match(restoreMeetingSql, /update public\.quality_task_templates[\s\S]+set[\s\S]+workstream\s*=\s*'quality'[\s\S]+active\s*=\s*true[\s\S]+superseded_by\s*=\s*null[\s\S]+source_key\s*=\s*'CBH-QT-42'/i, 'the original safety meeting is restored to the active Quality calendar')
assert.match(restoreMeetingSql, /update public\.quality_task_templates[\s\S]+active\s*=\s*false[\s\S]+superseded_by\s*=\s*primary_meeting\.id[\s\S]+source_key\s*=\s*'CBH-ST-05'/i, 'the duplicate Safety meeting is retired and linked to the Quality primary')
