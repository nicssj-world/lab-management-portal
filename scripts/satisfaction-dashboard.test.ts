...
assert.ok(dashboardRoute.includes("requireSatisfaction('view')"))
assert.ok(commentsRoute.includes("requireSatisfaction('view')"))
...
assert.ok(css.includes('@media (max-width: 767px)') && css.includes('satisfaction-comment-card'))
assert.ok(css.includes('prefers-reduced-motion') && css.includes('comment-filter'))
