...
const complianceQuery = fs.readFileSync('lib/queries/kpi-compliance.ts', 'utf8')
...
assert.match(monitor, /const isNotOpen = period\.status === 'not_open'/)
assert.match(monitor, /disabled=\{isNotOpen\}/)
assert.match(monitor, /aria-disabled=\{isNotOpen\}/)
assert.match(monitor, /const showProgress = period\.required_count > 0/)
assert.match(monitor, /status === 'not_open' \? '--'/)
assert.match(complianceQuery, /const materializedPeriods = months/)
