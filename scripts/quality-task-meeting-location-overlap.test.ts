...
const correctiveMigration = read('supabase/migrations/20260904202420_quality_task_meeting_location_nullif_fix.sql')
const manualScript = read('scripts/quality-task-meeting-location-overlap.sql')

for (const sql of [migration, correctiveMigration, manualScript]) {
  ...
  assert.ok(!sql.toLowerCase().includes('pg_catalog.nullif'), 'NULLIF must not be schema-qualified as a catalog function')
  assert.ok(sql.includes('NULLIF(pg_catalog.btrim(NEW.meeting_location), \'\'::text) IS NULL'), 'missing new locations conflict with every location')
  assert.ok(sql.includes('NULLIF(pg_catalog.btrim(existing.meeting_location), \'\'::text) IS NULL'), 'missing existing locations conflict with every location')
