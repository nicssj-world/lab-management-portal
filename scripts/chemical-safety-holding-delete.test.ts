...
assert.doesNotMatch(cascadeMigration, /holding_delete_shared_dependency/i)
assert.match(cascadeMigration, /source_holding_id\s*=\s*NULL/i, 'shared source versions are detached from the deleted holding')
assert.match(cascadeMigration, /detachedSdsVersionIds/i, 'cascade reports retained versions detached from the deleted holding')
