...
assert.ok(outlabDashboard.includes('role="tablist"') && outlabDashboard.includes('<Icon name={tabIcons[key]}'), 'OUTLAB ต้องมีแท็บพร้อมไอคอน')
assert.ok(eqaDashboard.includes('role="tablist"') && eqaDashboard.includes('<Icon name={tabIcons[key]}'), 'EQA ต้องมีแท็บพร้อมไอคอน')
const riskClient = readFileSync('components/risk/RiskClient.tsx', 'utf8')
assert.ok(riskClient.includes('<PageHeader') && riskClient.includes('className="risk-tabs"'), 'โมดูลความเสี่ยงต้องใช้โครงหัวและแท็บเดียวกับแบบสำรวจ')
