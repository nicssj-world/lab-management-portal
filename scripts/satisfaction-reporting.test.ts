...
assert.ok(reportRoute.includes("requireSatisfaction('view')"))
assert.ok(reportRoute.includes(".eq('survey_id', campaign.survey_id)"), 'scopes prior-year comparison to the same survey')
assert.ok(!actions.includes("import * as XLSX from 'xlsx'"), 'Excel library is not part of the initial satisfaction bundle')
assert.ok(actions.includes("await import('xlsx')"), 'Excel library loads only when an export is requested')
...
assert.ok(commentExport.includes("requireSatisfaction('view')"))
assert.ok(kpiRoute.includes("requireSatisfaction('edit')"))
