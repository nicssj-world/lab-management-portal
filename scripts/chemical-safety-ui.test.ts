...
assert.ok(hubSource.includes('registrySummary.productCount.toLocaleString()'), 'registry UI must show the distinct chemical master count')
assert.ok(hubSource.includes('registrySummary.registryEntryCount.toLocaleString()'), 'registry UI must show the scoped registry entry count')
assert.ok(!hubSource.includes('แยกสารเคมีหลักกับรายการของงาน/คลัง'), 'registry summary must not repeat the explanatory heading')
assert.ok(!hubSource.includes('สารเคมีหลักหนึ่งรายการอาจมีหลายรายการของงาน/คลัง'), 'registry summary must not repeat the explanatory paragraph')
assert.ok(!hubSource.includes('สารในห้องเก็บสารเคมีเผยแพร่อัตโนมัติ'), 'registry page must not show the automatic-publication callout')
assert.ok(!hubSource.includes('แก้ไขแล้วมีผลทันที ไม่ต้องกดเผยแพร่ระดับงาน'), 'registry page must not show the publication instruction callout')
assert.ok(!hubSource.includes('chemical-section-count'), 'registry heading must not duplicate the total visible count')
assert.ok(hubSource.includes('chemical-registry-summary-metrics'), 'registry metrics must use a dedicated summary layout')
assert.ok(hubSource.includes("justifyContent: 'flex-start'"), 'registry metrics must align with the section content instead of floating right')
assert.ok(!hubSource.includes('chemical-filter-result'), 'filter panel must not repeat the registry total on the right')
...
assert.ok(hubSource.includes('role="group"') && hubSource.includes('การจัดการ ${row.canonicalName}'), 'action bar ต้องประกาศกลุ่มการจัดการสำหรับ screen reader')
assert.ok(hubSource.includes('registryActionIconButtonStyle'), 'ปุ่ม action ต้องใช้ขนาดไอคอนล้วนร่วมกัน')
assert.ok(hubSource.includes('aria-label={`เปิดรายละเอียดสาร ${row.canonicalName}`}'), 'ปุ่มรายละเอียดต้องมีชื่อที่เข้าถึงได้เมื่อซ่อนไอคอนเป็นหลัก')
assert.ok(hubSource.includes('aria-label={`${isInactive ? \'ตั้งสถานะเป็น Active\' : \'ตั้งสถานะเป็น Inactive\'} สำหรับ ${row.canonicalName}`}'), 'ปุ่มเปลี่ยนสถานะต้องมีชื่อที่เข้าถึงได้')
assert.doesNotMatch(hubSource, /chemical-registry-action-label-(?:long|short)/, 'action bar ไม่ควรแสดง label ข้างไอคอนอีก')
...
assert.ok(hubSource.includes('gap:8px'), 'action bar ต้องเว้นระยะระหว่าง hit area ให้แตะได้ไม่พลาด')
assert.ok(hubSource.includes('@media(pointer:coarse)'), 'action bar ต้องขยาย hit area บนอุปกรณ์ touch')
