...
assert.match(round, /kind:\s*z\.string\(\)\.trim\(\)\.min\(1\)\.optional\(\)/, 'closing accepts an explicit equipment kind')
assert.match(round, /closedKinds/, 'round close persists which equipment kinds have been closed')
assert.match(round, /กรุณาเลือกประเภทอุปกรณ์ก่อนปิดรอบ/, 'mixed-type rounds cannot be closed without choosing a kind')
