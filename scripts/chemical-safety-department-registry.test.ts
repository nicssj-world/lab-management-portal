...
assert.match(sdsOnlyRoute, /department_sds_read_only/i, 'SDS-only route is closed because registry is central')
assert.match(sdsOnlyRoute, /status:\s*410/, 'SDS-only route returns Gone')
...
assert.match(dedupMigration, /delete from public\.chemical_department_chemical_links/i, 'deduplication removes duplicate registry links first')
assert.match(dedupMigration, /delete from public\.chemical_department_sds/i, 'deduplication removes duplicate legacy SDS metadata')
assert.match(dedupMigration, /delete from public\.chemical_sds_versions/i, 'deduplication removes unreferenced duplicate SDS versions')
assert.match(dedupMigration, /delete from public\.chemical_sds_files/i, 'deduplication removes only orphaned SDS files')
assert.match(dedupMigration, /not exists[\s\S]*chemical_sds_versions[\s\S]*not exists[\s\S]*chemical_department_sds/i, 'file cleanup keeps files still referenced elsewhere')
assert.match(dedupMigration, /create unique index if not exists[\s\S]*chemical_department_chemical_links[\s\S]*holding_id/i, 'database enforces one department SDS link per holding')
...
assert.match(sdsMutationRoute, /department_sds_read_only/i, 'department SDS mutations are closed')
assert.match(sdsReplaceRoute, /department_sds_read_only/i, 'department SDS replacement is closed')
